...
function Timer()
{
    ...
    for (Inv = Instigator.Inventory; Inv != None; Inv = Inv.Inventory)
    {
        ...
        if (W != None)
        {
            if (W.bNoAmmoInstances && W.AmmoClass[0] != None && !class'MutLumpysRPG'.static.IsSuperWeaponAmmo(W.AmmoClass[0]))
            {
        ...
        {
            Ammo = Ammunition(Inv);
            if (Ammo != None && !class'MutLumpysRPG'.static.IsSuperWeaponAmmo(Ammo.Class))
