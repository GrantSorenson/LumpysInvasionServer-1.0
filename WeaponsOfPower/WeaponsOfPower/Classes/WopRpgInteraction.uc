	...
	if (RPGArtifact(ViewportOwner.Actor.Pawn.SelectedItem) != None) {
		...
		if (Transition != 0) {
			...
			if (Direction >= 0) {
				...
				firstInventory = FindPrevNthItem(WopStatsInv.DisplayItem, (numToShow / 2) - 1);
			}
...
function DrawItemDescription(
	...
	if (fInfoPosition > 0.0) {
		...
		Canvas.WrapStringToArray(description, descriptionText, iBoxWidth + 3 * YL, "|");
...
defaultproperties
{
    ItemsToShow=5
    ScrollSpeed=8.00
		RpgDescriptions(0)=(ArtifactClass=Class'UT2004RPG.ArtifactInvulnerability',Description= "While active you will be invincible but your adrenaline will drain quickly.||Note that falling off levels and into death volumes will still kill you.",)
		RpgDescriptions(1)=(ArtifactClass=Class'UT2004RPG.ArtifactFlight',Description="Use this and you'll be able to fly around the level while your adrenaline lasts.",)
		RpgDescriptions(2)=(ArtifactClass=Class'UT2004RPG.ArtifactTripleDamage',Description="Triple the damage dealt by your weapons while your adrenaline last.",)
		RpgDescriptions(3)=(ArtifactClass=Class'UT2004RPG.ArtifactLightningRod',Description="Shoots a bolt of lightning at any enemy that comes within your sights, dealing them damage.",)
		RpgDescriptions(4)=(ArtifactClass=Class'UT2004RPG.ArtifactTeleport',Description="Randomly teleports you around the levels for a quick escape.",)
		RpgDescriptions(5)=(ArtifactClass=Class'UT2004RPG.ArtifactMonsterSummon',Description="Summons a friendly monster that will fight on your behalf. Only you can kill your summoned monsters.||Monsters with green halos are yours, monsters with red are teammate summoned monsters.",)
