class ShieldedRedeemerRedeemerFire extends RedeemerFire;
...
function Projectile SpawnProjectile(
	...
	local ShieldedRedeemerRedeemerWarhead Warhead;
	local PlayerController                Possessor;

	Warhead = Weapon.Spawn(class'WeaponsOfPower.ShieldedRedeemerRedeemerWarhead', Instigator,, Start, Dir);

	if (Warhead == None) {
		Warhead = Weapon.Spawn(class'WeaponsOfPower.ShieldedRedeemerRedeemerWarhead', Instigator,, Instigator.Location, Dir);
	}
...
defaultproperties
{
    ProjectileClass=Class'ShieldedRedeemerRedeemerProjectile'
}
