class AwarenessInvasionHUD extends HUDCTeamDeathMatch;
...
simulated function ShowTeamScorePassA(
	Canvas C
) {
	...
	if (
		(OwnerStats != None) &&
		(OwnerStats.ShouldShowRadar() == true)
	) {
		return;
	}

	colorPulse = class'HudCDeathmatchHelper'.static.GetHudTeamColor(PawnOwner);

	RadarScale = Default.RadarScale * HUDScale;
	RadarWidth = 0.5 * RadarScale * C.ClipX;
	PulseWidth = RadarScale * C.ClipX;
	C.DrawColor = colorPulse;
	C.Style = ERenderStyle.STY_Translucent;

	PulseBrightness = FMax(0,(1 - 2*RadarPulse));
	C.DrawColor.R = PulseBrightness * colorPulse.R;
	C.DrawColor.G = PulseBrightness * colorPulse.G;
	C.DrawColor.B = PulseBrightness * colorPulse.B;
...
simulated function ShowTeamScorePassC(
	Canvas C
) {
	local Pawn P;
