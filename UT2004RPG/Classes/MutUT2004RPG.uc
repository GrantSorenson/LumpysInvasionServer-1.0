...
function bool CheckReplacement(Actor Other, out byte bSuperRelevant)
{
	...
	if (WeaponModifierChance > 0)
	{
		if (Other.IsA('WeaponLocker') && !Other.IsA('RPGWeaponLocker'))
		{
			...
			RPGLocker = RPGWeaponLocker(ReplaceWithActor(Other, "UT2004RPG.RPGWeaponLocker"));
			if (RPGLocker != None)
			{
...
function ModifyPlayer(Pawn Other)
{
	...
	else
	{
		...
		{
			data.Level = StartingLevel;
			data.PointsAvailable = PointsPerLevel * (StartingLevel - 1) + 2000;
			data.AdrenalineMax = 100;
			data.NeededExp = 1000.0;// cap our needed xp at 1k, FIXME: should relly set this as a config value
...
//lets not worry about validating stat points and hope our player base doesnt hack
...
static function FillPlayInfo(PlayInfo PlayInfo)
{
	...
	PlayInfo.AddSetting("UT2004RPG", "bFakeBotLevels", default.PropsDisplayText[i++], 4, 10, "Check");
...
defaultproperties
{
		 DefaultWeapons(0)="XWeapons.Translocator"
		 DefaultWeapons(1)="LumpyRPGWeapons.mp5Gun"
     ...
     WeaponModifiers(10)=(WeaponClass=Class'UT2004RPG.RW_Poison',Chance=2)
	 	 //WeaponModifiers(11)=(WeaponClass=Class'WeaponsOfPower.WeaponOfPower',Chance=4)
     Version=22
